...
export default function Inventory() {
  ...
  const closeModals = () => {
    setShowAddModal(false); setShowEditModal(false); setShowRestockModal(false);
    setEditingDrug(null); setFormData(emptyForm); setRestockData(emptyRestock);
  };
  ...
  const handleEdit = (drug) => {
    setFormData({ drug_name: drug.drug_name||'', category: drug.category||'', quantity_in_stock: drug.quantity_in_stock||'', reorder_level: drug.reorder_level||'', dosage_form: drug.unit||drug.dosage_form||'tablet', unit_price: drug.unit_price||'' });
    setEditingDrug(drug); setShowEditModal(true);
  };

  const handleRestock = (drug) => {
    setEditingDrug(drug); setRestockData(emptyRestock); setShowRestockModal(true);
  };
  ...
  const handleRestockSubmit = async (e) => {
    e.preventDefault();
    if (!restockData.quantity||!restockData.batch_no||!restockData.expiry_date) { alert('All restock fields are required.'); return; }
    try {
  ...
  const DrugForm = ({ isEdit=false }) => (
    <form onSubmit={handleSubmit}>
      <div className="ab-modal-body">
        <div className="ab-form-row">
          <div className="ab-form-group">
            <label>Drug Name *</label>
            <input type="text" required value={formData.drug_name} onChange={e=>setFormData({...formData,drug_name:e.target.value})} placeholder="e.g. Amoxicillin 500mg"/>
          </div>
          <div className="ab-form-group">
        ...
        <div className="ab-form-row">
          <div className="ab-form-group">
            ...
            <input type="number" min="0" required value={formData.quantity_in_stock} onChange={e=>setFormData({...formData,quantity_in_stock:e.target.value})} placeholder="e.g. 100"/>
          </div>
          <div className="ab-form-group">
            <label>Reorder Level *</label>
            <input type="number" min="0" required value={formData.reorder_level} onChange={e=>setFormData({...formData,reorder_level:e.target.value})} placeholder="e.g. 20"/>
          </div>
        </div>
        <div className="ab-form-row">
          <div className="ab-form-group">
            ...
            <select value={formData.dosage_form} onChange={e=>setFormData({...formData,dosage_form:e.target.value})}>
              {DOSAGE_FORMS.map(f=><option key={f} value={f} style={{textTransform:"capitalize"}}>{f.charAt(0).toUpperCase()+f.slice(1)}</option>)}
            </select>
          </div>
          <div className="ab-form-group">
            <label>Unit Price (KES) *</label>
            <input type="number" min="0" step="0.01" required value={formData.unit_price} onChange={e=>setFormData({...formData,unit_price:e.target.value})} placeholder="e.g. 45.00"/>
          </div>
        </div>
      </div>
      <div className="ab-modal-footer">
        <button type="button" className="ab-btn-secondary" onClick={closeModals}>Cancel</button>
        <button type="submit" className="ab-btn-primary" disabled={isEdit?updating:creating}>
          <RiSaveLine size={14}/> {isEdit?(updating?"Updating…":"Update Drug"):(creating?"Adding…":"Add Drug")}
        </button>
      </div>
    </form>
  );
  ...
  return (
    <>
          ...
          <tbody>
            {tableLoading ? (
              Array.from({length:6}).map((_,i)=>(
                <tr key={i}>{Array.from({length:7}).map((_,j)=>(<td key={j}><div style={{height:14,background:"#f3f4f6",borderRadius:4}}/></td>))}</tr>
              ))
            ) : medicines.length === 0 ? (
              ...
              return (
                <tr key={drug.id} className={isCritical?"low-stock":""}>
                  ...
                  <td><span className="ab-cat-badge">{drug.category?drug.category.charAt(0).toUpperCase()+drug.category.slice(1):"—"}</span></td>
                  <td style={{ color:"var(--ab-slate-500)", fontSize:13, textTransform:"capitalize" }}>{drug.unit||drug.dosage_form||"—"}</td>
                  <td>
                  ...
                  <td style={{ fontWeight:600, color:"var(--ab-slate-800)" }}>KES {Number(drug.unit_price||0).toLocaleString("en-KE",{minimumFractionDigits:2})}</td>
                  <td>
      ...
      {/* Add Modal */}
      {showAddModal && <Modal title="Add New Drug" onClose={closeModals}><DrugForm/></Modal>}

      {/* Edit Modal */}
      {showEditModal && <Modal title="Edit Drug" onClose={closeModals}><DrugForm isEdit/></Modal>}
            ...
            <div className="ab-modal-body">
              ...
              <div className="ab-form-row">
                <div className="ab-form-group">
                  ...
                  <input type="number" min="1" required value={restockData.quantity} onChange={e=>setRestockData({...restockData,quantity:e.target.value})} placeholder="e.g. 100"/>
                </div>
                <div className="ab-form-group">
                  <label>Batch Number *</label>
                  <input type="text" required value={restockData.batch_no} onChange={e=>setRestockData({...restockData,batch_no:e.target.value})} placeholder="e.g. BATCH-2026-001"/>
                </div>
              </div>
              <div className="ab-form-group">
                <label>Expiry Date *</label>
                <input type="date" required value={restockData.expiry_date} onChange={e=>setRestockData({...restockData,expiry_date:e.target.value})}/>
              </div>
            </div>
            <div className="ab-modal-footer">
              <button type="button" className="ab-btn-secondary" onClick={closeModals}>Cancel</button>
