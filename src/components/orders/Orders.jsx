    ...
    <div className="ab-modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <div className="ab-modal-body">
          ...
          >
            Confirm that patient <strong>{order.patient_name}</strong> has been
            served. This will mark the order as <strong>delivered</strong> and
            update inventory.
          </p>
          <div
            style={{
          ...
          >
            ...
            {order.items?.map((it, i) => (
              ...
              >
                • {it.name || it.drug_name} — {it.dosage}, qty{" "}
                {it.quantity || "N/A"}
              </div>
            ))}
          </div>
        ...
        <div className="ab-modal-footer">
          <button className="ab-btn-secondary" onClick={onClose}>
            Cancel
          </button>
          <button
    ...
    <div className="ab-modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <div className="ab-modal-footer">
          ...
          <button
            className="ab-btn-primary"
            disabled={!riderId || loading || fetching}
      ...
      >
        ...
        <div className="ab-modal-body">
          ...
          >
            ...
            This will deduct stock and mark the order as{" "}
...
export default function Orders() {
  ...
  // All orders fetched once from /orders/available
  ...
  // Fetch allfetchOrder available orders from /orders/available — no params
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      // body is a plain array from this endpoint
  ...
  // Reset to page 1 when tab/search/priority changes
  useEffect(() => {
    setPage(1);
  ...
  const filtered = allOrders.filter((o) => {
    ...
    if (search && !o.patient_name?.toLowerCase().includes(search.toLowerCase()))
      return false;
    return true;
  });

  // Tab counts derived from allOrders
  ...
  const getPrescriptionItems = (order) => order.prescription?.items || [];
    ...
    <>
      <div className="ab-page-header">
        <div className="ab-page-title">
          ...
          <p>
            Fulfill prescription orders — dispense pickups or assign delivery
            riders.
          </p>
        </div>
      ...
      <div className="ab-tabs">
        {STATUS_TABS.map((s) => (
          ...
          >
            {TAB_LABELS[s]}{" "}
            <span className="ab-tab-count">{counts[s] ?? 0}</span>
          </div>
        ))}
      </div>
                ...
                return (
                  <tr key={order.id}>
                    <td>
                      ...
                      <div
                        style={{ fontSize: 11, color: "var(--ab-slate-400)" }}
                      >
                    ...
                    <td>
                      <span
                        className={`ab-priority ${order.priority || "normal"}`}
                      >
                    ...
                    <td>
                      <span
                        className={`ab-status ${STATUS_CLS[order.status] || "pending"}`}
                      >
            ...
            <div className="ab-page-btns">
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1)
                ...
                .map((n) => (
                  <button
                    key={n}
                    className={`ab-page-btn ${page === n ? "active" : ""}`}
                    onClick={() => setPage(n)}
                  >
                    {n}
                  </button>
      ...
      {riderTarget && (
        <AssignRiderModal
          ...
          onSuccess={(data) => {
            alert(
              `Rider assigned! OTP: ${data.otp_code} · Package: ${data.package_number}`,
            );
            setRiderTarget(null);
            fetchOrders();
          }}
