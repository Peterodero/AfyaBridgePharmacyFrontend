...
import { RiDownloadLine, RiCalendarLine, RiMapPinLine, RiCheckboxCircleLine, RiRefreshLine } from "react-icons/ri";
import { useApi } from "../../hooks/useApi";
import { getDashboardReport, getInvDashboard, getLowStock, getOrders, getDeliveries } from "../../services/api";
    ...
    <div style={{ display:"flex", alignItems:"flex-end", gap:6, height, paddingTop:8 }}>
      {data.map((d,i) => (
        <div key={i} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:4 }}>
          <div style={{ width:"100%", background:color, borderRadius:"4px 4px 0 0", height: `${Math.max((d.value/max)*100,4)}%`, opacity: 0.85+i*0.02 }} />
    ...
    <div style={{ display:"flex", alignItems:"center", gap:24 }}>
      <svg width={140} height={140} viewBox="0 0 140 140" style={{ flexShrink:0 }}>
        ...
        <text x={cx} y={cy-6} textAnchor="middle" fontSize="18" fontWeight="800" fill="#0f172a">
          {segments.reduce((s,d)=>s+d.rawValue,0)}
        </text>
...
export default function Analytics() {
  ...
  const { data: ordersData } = useApi(getOrders, { page_size:100 }, [], { silent: true });
  ...
  const kpiCards = [
    {
      ...
      value: invLoading ? "…" : String(inv.low_stock_count ?? lowStock.length ?? 0),
      badge: inv.low_stock_count>0?"HIGH":"OK",
      badgeColor: inv.low_stock_count>0?"var(--ab-red)":"var(--ab-green)",
    },
    {
    ...
    },
    {
      ...
      value: dashLoading?"…":`KES ${Number(stats.today_revenue||0).toLocaleString("en-KE",{minimumFractionDigits:0})}`,
    ...
    <>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16, marginBottom:24 }}>
        {kpiCards.map((k,i) => (
          <div key={i} className="ab-card" style={{ padding:"20px 24px" }}>
            ...
            <div style={{ fontSize:12, color:"var(--ab-slate-500)" }}>{k.label}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr minmax(280px,360px)", gap:20, marginBottom:20 }}>
        ...
        <div className="ab-card" style={{ padding:24 }}>
          <div style={{ marginBottom:16 }}>
            ...
            <div style={{ fontSize:12, color:"var(--ab-slate-400)", marginTop:2 }}>Current distribution across all order statuses</div>
          </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
        ...
        <div className="ab-card" style={{ padding:24 }}>
          ...
          {[
            { label:"Active Deliveries",   value:activeDeliveries, target:`Target: <${stats.active_deliveries??10}`, pct:activeDeliveries>0?Math.min((activeDeliveries/Math.max(doneDeliveries+activeDeliveries,1))*100,100):0, color:"var(--ab-blue)" },
            { label:"Completed Today",     value:doneDeliveries,   target:"Successfully delivered", pct:deliveries.length?Math.round((doneDeliveries/deliveries.length)*100):0, color:"var(--ab-green)" },
          ].map((item,i) => (
