...
import {
  RiRefreshLine, RiSearchLine, RiUserLine,
  RiCheckLine, RiCloseLine, RiTruckLine, RiAlertLine,
  RiEyeLine, RiFileListLine, RiUserStarLine,
} from "react-icons/ri";
import client, { getOrders, updateOrderStatus, cancelOrder, dispenseOrder, assignRiderToOrder, getAvailableRiders, serveOrder } from "../../services/api";
import OrderDetailsModal from "./OrderDetailsModal";

const PAGE_SIZE = 8;

// Removed "processing" and "dispatched" - keeping pending, ready, delivered, cancelled
      ...
      <div className="ab-modal" style={{ maxWidth:440 }} onClick={e=>e.stopPropagation()}>
        ...
        <div className="ab-modal-body">
          ...
          <div style={{background:"var(--ab-slate-50)",borderRadius:8,padding:"10px 14px",marginBottom:16}}>
            <div style={{fontSize:13,fontWeight:600,color:"var(--ab-slate-700)",marginBottom:8}}>
              Order Summary:
            </div>
        ...
        <div className="ab-modal-footer">
          ...
          <button className="ab-btn-primary" disabled={loading} onClick={handleServe}
      ...
      <div className="ab-modal" style={{ maxWidth:480 }} onClick={e=>e.stopPropagation()}>
        ...
        <div className="ab-modal-body">
          <div className="ab-form-group" style={{ marginBottom:16 }}>
            ...
            {fetching ? <div style={{fontSize:13,color:"var(--ab-slate-400)"}}>Loading riders…</div> :
              riders.length === 0 ? <div style={{fontSize:13,color:"#dc2626"}}>No riders currently on duty.</div> :
  ...
  return (
    <div className="ab-modal-overlay" onClick={onClose}>
      <div className="ab-modal" style={{ maxWidth:440 }} onClick={e=>e.stopPropagation()}>
        ...
        <div className="ab-modal-footer">
          <button className="ab-btn-secondary" onClick={onClose}>Cancel</button>
          <button className="ab-btn-primary" disabled={loading} onClick={handleDispense}
            style={{background:"#059669"}}>
...
export default function Orders() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      const params = { status: activeTab, page, page_size: PAGE_SIZE };
      if (search)   params.q        = search;
      if (priority) params.priority = priority;
      const res  = await getOrders(params);
      const body = res.data?.data ?? res.data;
      console.log("Fetched orders:", body);
      if (body?.results) { setOrders(body.results); setTotal(body.count ?? body.results.length); }
      else { setOrders(Array.isArray(body)?body:[]); setTotal(Array.isArray(body)?body.length:0); }
    } catch(e) {
      const msg = e.response?.data?.message || e.message || "Failed to load orders";
      setError(msg);
    } finally { setLoading(false); }
  }, [activeTab, page, search, priority]);

  const fetchCounts = useCallback(async () => {
    try {
      const results = await Promise.allSettled(
        STATUS_TABS.map(s => client.get(`/orders/?status=${s}&page_size=1`))
      );
      const nc = {};
      STATUS_TABS.forEach((s,i) => {
        if (results[i].status === "fulfilled") {
          const b = results[i].value.data?.data ?? results[i].value.data;
          nc[s] = b?.count ?? (Array.isArray(b)?b.length:0);
        } else { nc[s] = counts[s] ?? 0; }
      });
      setCounts(nc);
  ...
  function ActionCell({ order }) {
    ...
    return (
      <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
        ...
        {/* Serve button for pending orders */}
        {s === "pending" && (
          <button 
            className="ab-btn-primary" 
            style={{ fontSize:12, padding:"5px 12px", background:"#059669" }}
            disabled={busy} 
            onClick={() => setServeTarget(order)}
          >
        ...
        {s === "pending" && (
          <button 
            style={{ fontSize:12, padding:"5px 10px", background:"#fee2e2", color:"#991b1b", border:"none", borderRadius:6, cursor:"pointer" }}
            disabled={busy} 
        ...
        {/* Mark Ready button for ready orders - can move to delivered */}
        {s === "ready" && (
          <button 
            className="ab-btn-primary" 
        ...
        {/* Dispense button for ready pickup orders */}
        ...
        {/* Assign Rider button for ready delivery orders */}
        {/* {s === "ready" && order.delivery_type !== "pickup" && (
          <button 
            className="ab-btn-primary" 
            style={{ fontSize:12, padding:"5px 12px" }}
            disabled={busy} 
            onClick={() => setRiderTarget(order)}
          >
            <RiTruckLine size={12}/> Assign Rider
          </button>
        )} */}
      </div>
    );
  ...
  // Helper function to get prescription items
  const getPrescriptionItems = (order) => {
    return order.prescription?.items || [];
  };

  // Helper function to render medication column
  const renderMedicationColumn = (order) => {
    const items = getPrescriptionItems(order);
      ...
      return (
        <>
          ...
          <div style={{ fontSize: 11, color: "var(--ab-slate-400)" }}>
            {firstItem.dosage || ""}
            {firstItem.frequency && ` · ${firstItem.frequency}`}
            {firstItem.duration && ` · ${firstItem.duration}`}
            {items.length > 1 && (
  ...
  const getDeliveryTypeDisplay = (order) => {
  ...
  // Helper function to get delivery type styles
  const getDeliveryTypeStyles = (order) => {
    const isPickup = order.delivery_type === "pickup";
    return {
      background: isPickup ? "#eff6ff" : "#f5f3ff",
      color: isPickup ? "#1152d4" : "#7c3aed"
    };
  };
    ...
    <>
      <div className="ab-page-header">
        ...
        <div style={{ display:"flex", gap:8 }}>
          <button className="ab-btn-secondary" onClick={()=>{fetchOrders();fetchCounts();}}>
            <RiRefreshLine size={15}/> Refresh
          </button>
        </div>
      </div>

      {/* Tabs - Only pending, ready, delivered, cancelled */}
                  ...
                  <tr key={order.id}>
                    ...
                    <td>
                      <span style={{ 
                        ...
                        padding: "3px 8px", 
                        borderRadius: 6,
                        background: deliveryStyles.background,
                        color: deliveryStyles.color 
                      }}>
                        {deliveryDisplay}
                      </span>
                    </td>
      ...
      {/* Order Details Modal */}
      {viewOrder && (
        <OrderDetailsModal order={viewOrder} onClose={() => setViewOrder(null)} TAB_LABELS={TAB_LABELS} />
      )}

      {/* Serve modal */}
      {serveTarget && (
        <ServeModal order={serveTarget} onClose={()=>setServeTarget(null)}
          onSuccess={()=>{ setServeTarget(null); fetchOrders(); fetchCounts(); }} />
      )}

      {/* Dispense modal */}
      {dispenseTarget && (
        <DispenseModal order={dispenseTarget} onClose={()=>setDispenseTarget(null)}
          onSuccess={()=>{ setDispenseTarget(null); fetchOrders(); fetchCounts(); }} />
      )}
      ...
      {riderTarget && (
        <AssignRiderModal order={riderTarget} onClose={()=>setRiderTarget(null)}
          onSuccess={(data)=>{
            alert(`Rider assigned! OTP: ${data.otp_code} · Package: ${data.package_number}`);
            setRiderTarget(null);
